...
//!
//! Output is printed to stdout in a parseable format.
...
fn fmt_throughput(d: Duration, n: usize) -> String {
    let secs = d.as_secs_f64();
    if secs > 0.0 {
        format!("{:.0} ops/sec", n as f64 / secs)
    } else {
        "n/a".to_string()
    }
}
...
impl BenchResult {
    fn print(&self) {
        println!(
            "  {:<45} n={:>7}  total={:>12}  throughput={:>15}",
            &self.name,
            self.n,
            fmt_duration(self.total),
            fmt_throughput(self.total, self.n),
        );
    }
}

fn bench<F: FnMut()>(name: &str, n: usize, mut f: F) -> BenchResult {
    // warmup
    f();
...
fn bench_btree_get_miss(tree: &BTreeIndex, n: usize) -> BenchResult {
    let start = Instant::now();
    for i in 0..n as i64 {
        let key = make_int_key(i + 1_000_000); // non-existent keys
        let _ = tree.get(&key);
    }
    let total = start.elapsed();
    BenchResult {
...
fn bench_btree_range(tree: &BTreeIndex, n: usize, range_size: usize) -> BenchResult {
    ...
    BenchResult {
        name: format!("BTreeIndex::range ({} entries, {}x)", range_size, iterations),
        n: iterations,
        total,
    }
}
...
fn bench_btree_scan_all(tree: &BTreeIndex, n: usize) -> BenchResult {
    ...
    let total = start.elapsed();
    assert_eq!(total_entries, n * iterations);
    BenchResult {
...
fn bench_btree_remove(tree: &mut BTreeIndex, n: usize) -> BenchResult {
    // Re-insert since remove is destructive
    for i in 0..n as i64 {
        let key = make_int_key(i);
        let path = format!("/r/{}", i);
        tree.insert(key, path).unwrap();
...
fn bench_btree_update(tree: &mut BTreeIndex, n: usize) -> BenchResult {
    // Re-insert since previous remove destroyed data
    for i in 0..n as i64 {
        let key = make_int_key(i);
        let path = format!("/r/{}", i);
        tree.insert(key, path).unwrap();
...
async fn bench_manager_insert(n: usize) -> BenchResult {
    let dir = std::env::temp_dir().join(format!("rrdb_bench_insert_{}", n));
...
async fn bench_manager_range(manager: &IndexManager, n: usize, range_size: usize) -> BenchResult {
    ...
    assert!(total_entries > 0);
    BenchResult {
        name: format!("IndexManager::range ({} entries, {}x)", range_size, iterations),
        n: iterations,
        total,
    }
}
...
async fn bench_manager_persist_and_reload(n: usize) -> BenchResult {
    let dir = std::env::temp_dir().join(format!("rrdb_bench_reload_{}", n));
    let _ = tokio::fs::remove_dir_all(&dir).await;
    tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    {
        let manager = IndexManager::new(dir.clone());
        let db_dir = dir.join("benchdb").join("tables").join("benchtable").join("index");
        manager.load_all(&db_dir).await.unwrap();
        assert_eq!(manager.len("bench_idx").await.unwrap(), n);
    }
    let total = start.elapsed();
...
fn main() {
    ...
    for &n in scales {
        let results: Vec<BenchResult> = {
            ...
            // Remove and update are destructive, run them last
            vec![
                bench_btree_remove(&mut tree, n),
                bench_btree_update(&mut tree, n),
            ]
        };
        for r in results {
            r.print();
    ...
    for &n in scales {
        rt.block_on(async {
            // Insert benchmark (creates its own dir, destructive)
            bench_manager_insert(n).await.print();

            // For read benchmarks, set up a persistent manager
            let dir = std::env::temp_dir().join(format!("rrdb_bench_read_{}", n));
            let _ = tokio::fs::remove_dir_all(&dir).await;
            tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    // ── Summary Table ──
    println!("━━━ Summary: Key Operations at Different Scales ━━━━━━━━━━━━━━━━━━━━\n");
    println!("  {:<20} {:>10} {:>12} {:>14}", "Operation", "Entries", "Total Time", "Ops/sec");
    println!("  {}", "-".repeat(60));

    // Quick summary: insert at each scale
    for &n in scales {
        let mut tree = BTreeIndex::new("id".to_string(), false);
        let start = Instant::now();
        for i in 0..n as i64 {
