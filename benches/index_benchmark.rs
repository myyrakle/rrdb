...
fn bench_btree_scan_all(tree: &BTreeIndex, n: usize) -> BenchResult {
    ...
    let total = start.elapsed();
    assert_eq!(total_entries, n * iterations, "scan_all should return all entries");
    BenchResult {
...
async fn bench_manager_scan_all(manager: &IndexManager, n: usize) -> BenchResult {
    ...
    let total = start.elapsed();
    assert_eq!(total_entries, n * iterations, "manager scan_all should return all");
    BenchResult {
...
fn main() {
    ...
    for &n in disk_scales {
        rt.block_on(async {
            ...
            let dir = std::env::temp_dir().join(format!("rrdb_bench_read_{}_{}", n, std::process::id()));
            let _ = tokio::fs::remove_dir_all(&dir).await;
            tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    println!();
    println!("  {:<30} {:>10} {:>12} {:>16}", "Operation", "Entries", "Total Time", "Ops/sec");
    println!("  {}", "-".repeat(72));
