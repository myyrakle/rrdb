...
impl Parser {
    pub(crate) fn parse_begin_query(
        ...
        if !self.has_next_token() {
            return Err(Errors::new(ErrorKind::ParsingError("E2001 need more tokens".to_string())));
        }

        let current_token = self.get_next_token();

        if current_token != Token::Transaction {
            return Err(Errors::new(ErrorKind::ParsingError("E2002 Expected BEGIN".to_string())));
        }
