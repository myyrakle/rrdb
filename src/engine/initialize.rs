...
impl DBEngine {
    ...
    // 기본 설정파일 세팅
    async fn init_config(&self) -> Result<(), Errors> {
        // 1. 루트 디렉터리 생성 (없다면)
    ...
    async fn init_database(&self) -> Result<(), Errors> {
        // 6. 기본 데이터베이스 생성 (rrdb)
    ...
    async fn create_top_level_directory_if_not_exists(&self) -> Result<(), Errors> {
    ...
    async fn create_global_config_if_not_exists(&self) -> Result<(), Errors> {
    ...
    async fn create_data_directory_if_not_exists(&self) -> Result<(), Errors> {
    ...
    async fn create_wal_directory_if_not_exists(&self) -> Result<(), Errors> {
    ...
    #[cfg(target_os = "linux")]
    async fn create_daemon_config_if_not_exists(&self) -> Result<(), Errors> {
    ...
    #[allow(dead_code)]
    async fn write_and_check_err(&self, base_path: PathBuf, contents: &str) -> Result<(), Errors> {
        if let Err(error) = self
            .file_system
    ...
    async fn start_daemon(&self) -> Result<(), Errors> {
    ...
    fn check_output_status(&self, output: Result<Output, Error>) -> Result<(), Errors> {
        if output.is_err() {
